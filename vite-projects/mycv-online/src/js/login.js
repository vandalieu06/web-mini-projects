...
btnSubmit?.addEventListener("click", (e) => {
  ...
  const isValid = checkCredentials(email.value, password.value);

  const content = isValid ? "Todas la credenciales son correctas" : msgText;
  const colorToRemove = isValid ? "text-red-600" : "text-green-600";
  const colorToAdd = isValid ? "text-green-600" : "text-red-600";

  msgError.classList.remove(colorToRemove);
  msgError.classList.add(colorToAdd);
  msgError.innerHTML = content;

  if (isValid) window.location.href = "/src/pages/cv_form.html";
});
